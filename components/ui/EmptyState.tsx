...
export function EmptyState({
  ...
  return (
    <div className={`flex flex-col items-center gap-3 text-center ${compact?"py-8":"py-16"}`}>
      <div className={`flex items-center justify-center rounded-2xl border border-white/[0.06] bg-white/[0.02] ${compact?"h-10 w-10":"h-14 w-14"}`}>
        <Icon size={compact?20:24} className="text-white/30"/>
      </div>
      <div>
        <p className={`font-display font-semibold text-white/60 ${compact?"text-[13px]":"text-[15px]"}`}>{title}</p>
        {(body||hint)&&<p className="mt-1 max-w-xs text-[12px] text-white/25">{body||hint}</p>}
      </div>
      {cta&&(
        cta.href
