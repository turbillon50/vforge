...
export function OwnerPushBanner() {
  ...
  useEffect(() => {
    setMounted(true);
    try {
      if (localStorage.getItem("vforge_push_dismiss") === "1") {
        setDismissed(true);
        return;
      }
    } catch {
  ...
  return (
    <div
      className={`fixed right-2 z-40 flex w-[calc(100%-1rem)] max-w-[360px] items-center gap-2 border border-black bg-white p-2 shadow-[0_12px_32px_rgba(0,0,0,0.12)] sm:right-4 sm:w-auto sm:min-w-[320px] ${
        pathname === "/app/chat"
          ? "bottom-[calc(8rem+env(safe-area-inset-bottom,0px))] md:bottom-4"
    ...
    >
      ...
      <button
        type="button"
        className="grid h-8 w-8 shrink-0 place-items-center border border-[var(--border-1)] text-[12px]"
        aria-label="Cerrar avisos"
        onClick={() => {
          setDismissed(true);
          try {
            localStorage.setItem("vforge_push_dismiss", "1");
          } catch {
            /* ignore */
          }
        }}
