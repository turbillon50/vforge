...
const INTEGRATIONS = [
  ...
  },
  {
    name: "Stripe",
    color: "#6772e5",
    bg: "rgba(103,114,229,0.06)",
    border: "rgba(103,114,229,0.2)",
    icon: (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
  ...
  },
  {
    ...
    icon: (
      <svg width="22" height="22" viewBox="0 0 109 113" fill="none">
        ...
        <path d="M45.3 106.9c2.7 4.1 8.9 2.2 8.9-2.6V57H17.8c-6.5 0-10.1 7.7-6 12.4l33.5 37.5z" fill="url(#sb2)"/>
        <defs><linearGradient id="sb2" x1="22" y1="63" x2="61" y2="107" gradientUnits="userSpaceOnUse"><stop stopColor="#249361"/><stop offset="1" stopColor="#3ECF8E"/></linearGradient></defs>
      </svg>
    ),
  },
  {
  ...
  },
  {
    name: "Cloudflare",
    color: "#f6821f",
    bg: "rgba(246,130,31,0.06)",
    border: "rgba(246,130,31,0.2)",
    icon: (
      <svg width="28" height="18" viewBox="0 0 110 70" fill="none">
        ...
        <path d="M82 22c-.8-4.6-5-8-10-8-2.2 0-4.2.7-5.9 1.8C63.7 8.8 56.4 4 48 4c-13.3 0-24 10.7-24 24 0 .7 0 1.4.1 2C17 31.4 12 37 12 44c0 7.2 5.8 13 13 13h65c7.7 0 14-6.3 14-14 0-6.8-4.8-12.4-11.4-13.7C83.3 27.8 83 24.9 82 22z" fill="#fbad41" opacity="0.7"/>
      </svg>
    ),
  },
  {
  ...
  },
  {
    name: "Neon DB",
    color: "#00e5bf",
    bg: "rgba(0,229,191,0.06)",
    border: "rgba(0,229,191,0.2)",
    icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
        <rect x="2" y="2" width="20" height="20" rx="5" fill="#00e5bf" opacity="0.12"/>
  ...
  return (
    <section style={{ background:"linear-gradient(180deg, #06080f 0%, #050a14 100%)", padding:"80px 0", overflow:"hidden" }}>
      <div style={{ maxWidth:1200, margin:"0 auto", padding:"0 24px", marginBottom:48 }}>
        ...
        <p style={{ fontSize:16, color:"rgba(200,215,255,0.5)", lineHeight:1.65, maxWidth:420, marginTop:12 }}>
          Cada integracion nativa. Sin webhooks manuales. Sin configuracion extra.
        </p>
      </div>

      <div style={{ position:"relative", overflow:"hidden" }}>
        <div style={{ position:"absolute", left:0, top:0, bottom:0, width:160, background:"linear-gradient(to right, #06080f, transparent)", zIndex:2, pointerEvents:"none" }}/>
        <div style={{ position:"absolute", right:0, top:0, bottom:0, width:160, background:"linear-gradient(to left, #06080f, transparent)", zIndex:2, pointerEvents:"none" }}/>

        <div style={{
          display:"flex", gap:16, width:"max-content",
          animation:"vf-scroll-integ 35s linear infinite",
        }}>
          {[...INTEGRATIONS, ...INTEGRATIONS].map((item, i) => (
            <div key={i} style={{
              ...
              <span style={{ fontSize:15, fontWeight:500, color:"#c8d7ff", letterSpacing:"-0.02em" }}>
                {item.name}
              </span>
            </div>
          ))}
      ...
      <div style={{ position:"relative", overflow:"hidden", marginTop:16 }}>
        ...
        <div style={{
          display:"flex", gap:16, width:"max-content",
          animation:"vf-scroll-integ-r 40s linear infinite reverse",
        }}>
          {[...INTEGRATIONS.slice(4), ...INTEGRATIONS.slice(0,4), ...INTEGRATIONS.slice(4), ...INTEGRATIONS.slice(0,4)].map((item, i) => (
            <div key={i} style={{
              display:"inline-flex", alignItems:"center", gap:12,
              background: item.bg,
              border: `1px solid ${item.border}`,
      ...
      <style dangerouslySetInnerHTML={{ __html: "@keyframes vf-scroll-integ { from { transform: translateX(0); } to { transform: translateX(-50%); } } @keyframes vf-scroll-integ-r { from { transform: translateX(0); } to { transform: translateX(-50%); } }" }} />
