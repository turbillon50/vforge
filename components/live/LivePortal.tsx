...
function LiveWorkspace({
  ...
  const focusedContent = focusedPanel === "desktop" ? (
    ...
    <CommentsPanel projectId={project.id} projectName={project.name} workspace onFocus={() => setFocusedPanel(null)} focused />
  ) : null;
            ...
            <Panel id="feedback" minSize={150}>
              ...
              >
                ...
                <Panel id="comments" panelRef={commentsRef} collapsible collapsedSize={42} minSize={isMobile ? 120 : 180} onResize={(size) => updateCollapsed("comments", size.inPixels)}>
                  {collapsed.comments ? <CollapsedDockPanel label="Comentarios" vertical={!isMobile} onRestore={() => commentsRef.current?.expand()} /> : <CommentsPanel projectId={project.id} projectName={project.name} workspace onFocus={() => focusAction("comments")} onMinimize={() => commentsRef.current?.collapse()} />}
                </Panel>
              </Group>
            </Panel>
          </Group>
