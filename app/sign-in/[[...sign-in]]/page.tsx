...
export default function SignInPage() {
  ...
  return (
    <>
      <style>{`
        .si-root{background:#0A0A0A;min-height:100vh;display:flex;align-items:center;justify-content:center;font-family:Inter,-apple-system,sans-serif;position:relative}
        .si-root::before{content:"";position:absolute;inset:0;background-image:linear-gradient(to right,rgba(30,41,59,.08) 1px,transparent 1px),linear-gradient(to bottom,rgba(30,41,59,.08) 1px,transparent 1px);background-size:40px 40px;pointer-events:none}
        .si-mono{font-family:'JetBrains Mono',monospace}
        .si-scanline{position:absolute;top:0;left:0;width:100%;height:20px;background:linear-gradient(to bottom,transparent,rgba(0,240,255,.08),transparent);animation:si-scan 3s linear infinite;pointer-events:none}
        @keyframes si-scan{0%{transform:translateY(-100vh)}100%{transform:translateY(100vh)}}
        .si-corner::before{content:"";position:absolute;top:-1px;left:-1px;width:10px;height:10px;border-top:2px solid #00F0FF;border-left:2px solid #00F0FF}
        .si-corner::after{content:"";position:absolute;bottom:-1px;right:-1px;width:10px;height:10px;border-bottom:2px solid #00F0FF;border-right:2px solid #00F0FF}
      `}</style>
      <div className="si-root">
        ...
        <div style={{position:'relative',zIndex:10,width:'100%',maxWidth:400,padding:'0 20px'}}>
          <div style={{textAlign:'center',marginBottom:32}}>
            <p className="si-mono" style={{color:'#00F0FF',fontSize:12,letterSpacing:'0.1em',fontWeight:700,marginBottom:8}}>
            ...
            <h1 style={{color:'#fff',fontSize:32,fontWeight:700,fontFamily:"'Hanken Grotesk',sans-serif",letterSpacing:'-0.02em',marginBottom:8}}>
              Initialize <span style={{color:'#00F0FF'}}>VForge</span>
            </h1>
            <p style={{color:'#b9cacb',fontSize:14}}>Authenticate to access your production environment</p>
          </div>
          <div className="si-corner" style={{position:'relative',padding:4}}>
            {clerkEnabled ? (
              <>
                <ClerkLoading>
                  <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',minHeight:'40vh',gap:16,background:'#131313',padding:32}}>
                    <svg viewBox="0 0 24 24" width="34" height="34" fill="#00F0FF" style={{animation:'si-scan 2s ease infinite'}}>
                      <path d="M2 3h20L12 21z"/>
                    </svg>
                    <p className="si-mono" style={{color:'#849495',fontSize:10,letterSpacing:'0.3em'}}>INITIALIZING_</p>
                  </div>
                </ClerkLoading>
                <ClerkLoaded>
                  <SignIn appearance={kineticAppearance} />
                </ClerkLoaded>
              </>
            ) : (
