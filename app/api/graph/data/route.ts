 ...
 * Merges the DB projects catalog with live GitHub repos so the
 * component gets full info (deploy status, github link, health).
 */
...
export async function GET(req: Request) {
  ...
  const [dbProjects, ghRepos, vercelProjects] = await Promise.all([
    ...
    listAllUserRepos({ max: 200, auditUserId: auth.userId }),
    listProjects({ auditUserId: auth.userId }).catch(() => []),
  ]);

  // Build vercel status map by project name
  ...
  const vercelNodes = dbProjects.map(p => {
    ...
    return {
      ...
      data: {
        ...
        ghUrl: ghRepo?.html_url ?? (p.github_repo ? `https://github.com/${ghRepo?.full_name ?? p.github_repo}` : null),
  ...
  // GitHub-only nodes: repos not linked to any DB project
  ...
  const githubNodes = ghRepos
    ...
    .map(r => ({
      id: "g_" + r.full_name.replace("/", "_"),
      label: r.name,
      type: "github" as const,
      cat: guessCategory(r.name),
      dead: isDead(r.name, r.language) || r.archived,
      data: {
        dbId: null,
        name: r.name,
        framework: r.language,
        status: "GITHUB_ONLY",
        category: null,
        domain: null,
        vercelUrl: null,
  ...
  // Edges: fuzzy-match vercel ↔ github nodes
  ...
  return Response.json({
    ...
    edges,
    stats: {
      vercel: vercelNodes.length,
      github: githubNodes.length,
      edges: edges.length,
      dead: allNodes.filter(n => n.dead).length,
    },
  });
}
