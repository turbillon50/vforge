...
const ENDPOINTS = [
  { method: "POST", path: "/v1/chat", desc: "Envía un mensaje al agente V y recibe respuesta en streaming.", auth: true },
...
const SDKS = [
  ...
  { lang: "cURL", install: "nativo — sin instalación", color: "#10b981" },
];
...
const PILLARS = [
  {
    icon: "⚡",
    title: "MCP-nativo",
    desc: "Cada herramienta es un tool MCP. Conecta tu propio agente al Brain de VForge con una URL y un token.",
  },
  {
    icon: "🔁",
    title: "Webhooks en tiempo real",
    desc: "Recibe eventos de deploy, builds y commits al instante. Cero polling, cero latencia.",
  },
  {
    icon: "🔐",
    title: "Auth con Bearer token",
    desc: "Un token por workspace. Scope granular por recurso. Rotación sin downtime.",
  },
  {
    icon: "📦",
    title: "SDK tipado",
    desc: "TypeScript-first. Inferencia completa en cada respuesta. Sin any, sin sorpresas.",
  },
];
    ...
    <div style={{ background: "#020408", minHeight: "100vh", color: "#e2e8f0" }}>
      ...
      <section style={{ padding: "140px 24px 96px", maxWidth: 960, margin: "0 auto" }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.25)", borderRadius: 999, padding: "5px 14px", marginBottom: 28 }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 500, letterSpacing: "0.06em", textTransform: "uppercase", color: "#818cf8" }}>API pública · Beta</span>
        </div>
      ...
      <section style={{ borderTop: "1px solid rgba(255,255,255,0.05)", borderBottom: "1px solid rgba(255,255,255,0.05)", background: "rgba(255,255,255,0.015)", padding: "64px 24px" }}>
        <div style={{ maxWidth: 960, margin: "0 auto" }}>
          <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.15em", textTransform: "uppercase", color: "#475569", marginBottom: 12 }}>Quickstart</p>
          <h2 style={{ fontSize: 28, fontWeight: 700, letterSpacing: "-0.03em", color: "#f1f5f9", margin: "0 0 32px" }}>Tres líneas para empezar</h2>
          <div style={{ background: "#0d1117", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, overflow: "hidden" }}>
            ...
            <pre style={{ margin: 0, padding: "24px 28px", fontFamily: "'JetBrains Mono', 'Fira Code', monospace", fontSize: 14, lineHeight: 2, overflowX: "auto", color: "#e2e8f0" }}>
...
"}
              <span style={{ color: "#818cf8" }}>export</span> VFORGE_TOKEN=<span style={{ color: "#34d399" }}>vf_live_xxxxxxxxxxxxx</span>{"

"}
              <span style={{ color: "#475569" }}># 3. Conecta tu agente al MCP Brain</span>{"
"}
              <span style={{ color: "#818cf8" }}>npx</span> @vforge/cli mcp connect <span style={{ color: "#34d399" }}>--workspace my-org</span>
            </pre>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: "96px 24px" }}>
        <div style={{ maxWidth: 960, margin: "0 auto" }}>
          <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.15em", textTransform: "uppercase", color: "#475569", marginBottom: 12 }}>Diseñado para builders</p>
          <h2 style={{ fontSize: 28, fontWeight: 700, letterSpacing: "-0.03em", color: "#f1f5f9", margin: "0 0 48px" }}>Sin fricción. Sin boilerplate.</h2>
      ...
      <section id="endpoints" style={{ borderTop: "1px solid rgba(255,255,255,0.05)", padding: "96px 24px" }}>
        <div style={{ maxWidth: 960, margin: "0 auto" }}>
          <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.15em", textTransform: "uppercase", color: "#475569", marginBottom: 12 }}>REST API</p>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
            {ENDPOINTS.map((ep) => (
              <div key={ep.path} style={{ display: "flex", alignItems: "flex-start", gap: 20, padding: "20px 24px", background: "rgba(255,255,255,0.015)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 10, flexWrap: "wrap" }}>
                <span style={{
                ...
                {ep.auth && <span style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: "#a78bfa", background: "rgba(167,139,250,0.08)", border: "1px solid rgba(167,139,250,0.15)", padding: "2px 8px", borderRadius: 999 }}>Bearer</span>}
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ borderTop: "1px solid rgba(255,255,255,0.05)", background: "rgba(255,255,255,0.01)", padding: "96px 24px" }}>
        <div style={{ maxWidth: 960, margin: "0 auto" }}>
          <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.15em", textTransform: "uppercase", color: "#475569", marginBottom: 12 }}>SDKs</p>
      ...
      <section style={{ borderTop: "1px solid rgba(255,255,255,0.05)", padding: "96px 24px 120px" }}>
        <div style={{ maxWidth: 640, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 16, color: "#64748b", margin: "0 0 36px", lineHeight: 1.7 }}>
            Empieza con el plan gratuito. Sin tarjeta de crédito. API key en segundos.
          </p>
          <Link href="/sign-up" style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "14px 32px", borderRadius: 12, background: "linear-gradient(135deg, #6366f1, #8b5cf6)", color: "#fff", fontWeight: 700, fontSize: 16, textDecoration: "none", letterSpacing: "-0.01em" }}>
            Crear cuenta gratis →
          </Link>
        </div>
      </section>
