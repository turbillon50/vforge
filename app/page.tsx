...
// → crea PR, corre build, asigna dominio`,
  ...
  },
  {
    ...
    code: `// Acceso seguro desde cualquier proyecto
const key = await vault.get("STRIPE_KEY", {
  ...
  },
  {
    ...
    desc: "WhatsApp inbound, crons de mantenimiento, bridges con Vercel y GitHub — todo en un lugar.",
    code: `// Flow de notificación
flow
  .on("deploy:success", async (ctx) => {
...
const STEPS = [
  { n: "01", title: "Crea tu cuenta", desc: "Sign up en segundos. Sin tarjeta de crédito para empezar." },
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Gestiona proyectos, conversa con tu AI copilot, controla secrets y
          automatiza deployments — todo desde un solo workspace.
        </p>
      ...
      >
        {FEATURES.map((f) => (
          ...
          >
            ...
            >
              <code style={{ color: `${f.accent}cc` }}>{f.code}</code>
            </pre>
          </div>
        ))}
      </section>
      ...
      >
        <span style={{ fontSize: "12px", color: "#222", fontFamily: "monospace" }}>
          VForge · All Global Holding LLC
        </span>
