      ...
      <div className="mx-auto max-w-2xl space-y-5 px-5 pt-8">
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{ease:EASE}}
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            ...
            <a href="/app/chat" className="flex items-center gap-1.5 rounded-xl bg-white/[0.06] px-3 py-2 text-xs font-medium text-white/70 transition hover:bg-white/[0.12]">
          ...
          <div className="space-y-3">
            {MILESTONES.map(m=>(
              <div key={m.id} className="flex items-center gap-3">
                <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full border text-xs ${m.status==="done"?"border-emerald-400/40 bg-emerald-400/10 text-emerald-400":m.status==="active"?"border-violet-400/50 bg-violet-400/15 text-violet-300":"border-white/10 bg-white/[0.03] text-white/25"}`}>
          ...
          <div className="space-y-3">
            {UPDATES.map((u,i)=>(
              <div key={i} className="flex gap-3">
                ...
                <div>
                  <p className="text-sm font-light leading-relaxed text-white/70">{u.msg}</p>
        ...
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.3,ease:EASE}}
          ...
          <IconShield size={13} className="shrink-0 text-white/20"/>
          <p className="text-xs font-light text-white/25">Tu acceso está limitado a los recursos de este proyecto.</p>
        </motion.div>
      </div>
    </div>
  );
