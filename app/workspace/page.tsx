...
export default function ClientWorkspacePage() {
  ...
  const handleShare = () => {
    if (navigator.share) navigator.share({ title:"Mi proyecto en VForge", url:shareUrl });
      ...
      <div className="mx-auto max-w-2xl space-y-5 px-5 pt-8">
        ...
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{ease:EASE}}
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            ...
            <a href="#" className="flex items-center gap-1.5 rounded-xl bg-white/[0.06] px-3 py-2 text-xs font-medium text-white/70 transition hover:bg-white/[0.12]">
              <IconBranch size={12} className="text-violet-300"/> Ver repositorio
            </a>
        ...
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.3,ease:EASE}}
          ...
          <p className="text-xs font-light text-white/25">Tu acceso está limitado a los recursos de este proyecto. Gestionado por tu proveedor VForge.</p>
        </motion.div>
      </div>
    </div>
  );
