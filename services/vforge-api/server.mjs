...
const cartComponentIdMaxLength = 80;
const cartComponentNameMaxLength = 160;
const cartComponentKindMaxLength = 40;
...
function projectRoute(pathname) {
  const match = /^\/api\/v1\/projects\/([^/]+)\/(live|events|events\/stream|comments|context|assets|cart)(?:\/([^/]+))?$/.exec(
    pathname,
  );
  ...
  const subId = match[3] || null;
  if (subId && match[2] !== "assets" && match[2] !== "cart") return null;
  if (subId && !/^[0-9a-f-]{36}$/i.test(subId)) return null;
  return { projectId, resource: match[2], assetId: subId };
}
...
async function liveCart(req, res, projectId, itemId) {
  const access = await authorizeProject(req, res, projectId);
  if (!access) return;

  if (req.method === "GET") {
    const items = await sql.query(
      `SELECT id, component_id, component_name, component_kind, added_by_email, added_at
         FROM project_component_cart
        WHERE project_id = $1
        ORDER BY added_at DESC`,
      [projectId],
    );
    json(res, 200, { items, canWrite: canWriteContext(access.role) });
    return;
  }
  ...
  if (req.method === "DELETE") {
    ...
    await sql.query(
      `DELETE FROM project_component_cart WHERE id = $1 AND project_id = $2`,
      [itemId, projectId],
    );
  ...
  const payload = await readJsonBody(req);
  const componentId = payload && typeof payload === "object" ? payload.componentId : null;
  const componentName = payload && typeof payload === "object" ? payload.componentName : null;
  const rawKind = payload && typeof payload === "object" ? payload.componentKind : null;
  const componentKind = rawKind == null ? null : rawKind;
  if (
    typeof componentId !== "string" || !componentId.trim() || componentId.trim().length > cartComponentIdMaxLength ||
    typeof componentName !== "string" || !componentName.trim() || componentName.trim().length > cartComponentNameMaxLength ||
    (componentKind != null && (typeof componentKind !== "string" || componentKind.length > cartComponentKindMaxLength))
  ) {
    json(res, 400, { error: "invalid_component" });
    return;
  }

  const rows = await sql.query(
    `WITH created AS (
       INSERT INTO project_component_cart
         (project_id, component_id, component_name, component_kind, added_by_email)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (project_id, component_id) DO UPDATE
         SET added_at = now(), added_by_email = EXCLUDED.added_by_email
       RETURNING id, component_id, component_name, component_kind, added_by_email, added_at
     ), activity AS (
       INSERT INTO project_events (project_id, event_type, details, severity)
       SELECT $1, 'cart.component_added',
              jsonb_build_object('message', 'Componente agregado al carrito: ' || component_name, 'component_id', component_id),
              'low'
         FROM created
     )
     SELECT id, component_id, component_name, component_kind, added_by_email, added_at FROM created`,
    [
      projectId,
      componentId.trim(),
      componentName.trim(),
      componentKind ? componentKind.trim() : null,
      access.email,
    ],
  );

  json(res, 201, { item: rows[0] });
}
...
async function handle(req, res) {
  ...
  if (route.resource === "cart") {
    if (req.method !== "GET" && req.method !== "POST" && req.method !== "DELETE") {
      methodNotAllowed(res, "GET, POST, DELETE");
      return;
    }
    await liveCart(req, res, route.projectId, route.assetId);
    return;
  }
